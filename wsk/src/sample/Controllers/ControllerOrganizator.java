...
public class ControllerOrganizator {
    ...
    private void showMyInfo(int Code) {
        try {
            Connection conn = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/worldskilscourse4",
                    "root", "root");
            Statement statement = conn.createStatement();
    ...
    public void moderCombo() {
        try (Connection conn = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/worldskilscourse4",
                "root", "root")) {
            Statement statement = conn.createStatement();
            ResultSet resultSet = statement.executeQuery("SELECT ФИО FROM moders");
            while (resultSet.next()){
